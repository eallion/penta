...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('imageFile').addEventListener('change', async (e) => {
    ...
    const dateVal = document.getElementById('pentaDate').value;
    const champion = document.getElementById('championName').value;
    const ext = file.name.split('.').pop() || 'jpg';
    const dateStr = dateVal
      ? dateVal.replace(/[-]/g, '').slice(0, 8)
      : new Date().toISOString().slice(0, 10).replace(/-/g, '');
    const filename = `${dateStr}-${champion || 'unknown'}.${ext}`;

    try {
      const result = await uploadToCos(file, token, filename);
  ...
  document.getElementById('heroId').addEventListener('blur', autoFillChampion);
  document.getElementById('heroId').addEventListener('keydown', (e) => {
    if (e.key === 'Enter') autoFillChampion();
  });
...
async function autoFillChampion() {
  const heroId = document.getElementById('heroId').value.trim();
  if (!heroId) return;
  try {
    const res = await fetch(`/api/lol/champion/${heroId}`);
    if (!res.ok) return;
    const data = await res.json();
    if (data.id) document.getElementById('championName').value = data.id;
    if (data.nameZh) document.getElementById('championZh').value = data.nameZh;
    if (data.titleZh) document.getElementById('championTitle').value = data.titleZh;
  } catch (e) {}
}
